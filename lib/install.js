...
function InstallInit(){
    ...
    this.mname = this.patt1.exec(argv.install)[1]
    
    this.install = function(name){ //the Main install function
        var that = this
                if (typeof name == 'string'){
                    argv.install = name
                    that.mname = that.patt1.exec(argv.install)[1]
                    console.log('change to '+ that.mname)
                }
        ...
        fs.readdir(process.cwd() + '/modules/'+ that.mname, function(err,files){
            if (typeof files === 'object')
                ...
                if (typeof argv.install === 'boolean')
                    ...
                    that.installModule(argv.install,argv.save,argv.dirty)
            }
        })   
    }
